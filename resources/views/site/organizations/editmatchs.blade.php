...
<body style="background-color: #56606B;">
    ...
    <div style="color: #fff;">
        <div>
            <h1 style="padding: 40px 0 10px 0; text-align: center;">EDITAR PARTIDA</h1>
            <main class="form-signin">
                <div id="event-create-container">
                    <form action="/editmatchs" method="POST">
                        @csrf
                        <?php
                        ...
                        <div style="display:none;">
                            ...
                            <input type="number" id="idTt" name="idTt" value="{{$matchs[$aux]->id}}">
                        </div>
                        <div class="form-group">
                            <label for="title">Time A:</label>
                            <select class="form-control" name="teama" id="teama" required>
                                @foreach($team as $x)
                                <option value="{{$x->id}}" <?php if($x->id == $matchs[$aux]->team_id_A){echo 'selected';} ?>>{{$x->name}}</option>
                                @endforeach
                            </select>
                        </div><br>
                        <div class="form-group">
                            ...
                            <select class="form-control" name="teamb" id="teamb" required>
                                @foreach($team as $x)
                                <option value="{{$x->id}}" <?php if($x->id == $matchs[$aux]->team_id_B){echo 'selected';} ?>>{{$x->name}}</option>
                                @endforeach
                            </select>
                        </div><br>
                        <div class="form-group">
                            <label for="title">Resultado:</label>
                            <input type="text" style="display:none;" name="rresult" id="rresult" value="{{$matchs[$aux]->result}}">
                            <select class="form-control" name="result" id="result" required>
                                <option id="empate" value="0">Empate</option>
                                <option id="vitoriaa" value="1">Vitória A</option>
                                <option id="vitoriab" value="2">Vitória B</option>
                                <option id="nothing" value="3">Indefinido</option>
                            </select><br>
                        </div><br>
